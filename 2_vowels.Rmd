...
## Absolute values in (medial position) for high vowels
### CD (medial position)
```{r}
final_df %>%
  filter(
  ...
  geom_text(data = high_vowels, aes(y = 430, label = stop_label), alpha = 0.1)+
  geom_boxplot()+
  geom_point(data = high_vowels, alpha = 0.8,
               aes(c_type, closdur, color = folVowel))+
  theme_bw()+
...
final_df %>% 
  ...
  ggplot(aes(closdur, VOT, label = stop))+
  geom_text(color = "grey")+
  geom_text(data = high_vowels, aes(closdur, VOT, label = stop_label, color = folVowel))+
  theme_bw()+
  theme(legend.position="bottom")+
  geom_hline(yintercept = 30, lty = 2)+
  geom_hline(yintercept = 160, lty = 2)+
